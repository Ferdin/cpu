...
public class TestGame extends JPanel implements KeyListener {
    ...
    public boolean updateScreen() {
        ...
        for (int addr = 0x0200; addr < 0x0600; addr++) {
            int colorIdx = cpu.memRead(addr) & 0xFF;
        ...
        new Thread(() -> {
            ...
            while (true) {
                ...
                while (cyclesThisFrame < 500) { // small batch, not full CYCLES_PER_FRAME
                    int op = cpu.memRead(cpu.getProgramCounter()) & 0xFF;
                    if (op == 0x00) return;

                    cpu.memWrite(0xFF, panel.currentInput);
                    cpu.memWrite(0xFE, (byte)(rng.nextInt(15) + 1));
